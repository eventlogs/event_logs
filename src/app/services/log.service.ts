...
export class LogService {
    ...
    private static getAttributeValueAsString(attribute: EventLogAttribute | number | string): string {
        ...
        if (attribute instanceof IntAttribute || attribute instanceof FloatAttribute) {
    ...
    public generate(eventLog: EventLog): string {
        ...
        logString += (this._caseIdElement + '\n' + this._activityElement + '\n' + attributes.join("\n") + '\n');

        logString += '.events' + '\n';
        logString += traces
            .map((trace, caseId) => this.getTraceRepresentation(trace, caseId, attributes))
            .join('\n');
        return logString;
    }
    ...
    private listOfAttributes(traces: Trace[]): string[] {
        ...
        const attributesOccurrencesMap = attributesWithDuplicates
    ...
    private getTraceRepresentation(trace: Trace, caseId: number, attributes: string[]): string {
        return trace.events.map(event => this.getEventRepresentation(event, caseId, attributes)).join('\n');
    }

    private getEventRepresentation(event: Event, caseId: number, attributes: string[]): string {
        const otherAttributes = attributes.map(attributeKey => event.getAttribute(attributeKey))
        const eventAttributes = [caseId, event.activity.toString()].concat(otherAttributes);
        const attributesAsStrings = eventAttributes
            .map(attribute => attribute == null ? this._undefinedValue : LogService.getAttributeValueAsString(attribute))
    ...
    private toRepresentationLine(attributesAsStrings: string[]): string {
        return attributesAsStrings.reduceRight((acc, e) => {
            if (acc === this._undefinedValue) {
                return (e === this._undefinedValue ? this._undefinedValue : this.escapeIfNecessary(e));
            }
