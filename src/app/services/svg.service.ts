...
export class SvgService {
                        ...
                        (svgEl) => {
                            ...
                            } else if ( svgEl.type === ElementType.box )
                            {
                                if (!this.activityColorMap.has(ev.activity)) {
                                    this.activityColorMap.set( ev.activity,
                                         this.activityColorMap.size % this.backgroundColors.length) // Wenn die Map größer ist als Size Farben recyclen
                                }
                                const rect = this.createBoxForElement( ev.svgElements[1], this.activityColorMap.get(ev.activity)! );
                                result.push( rect );
                            }
                        }
                    )
    ...
    private createBoxForElement(element: Element, fill: number) : SVGElement {
        ...
        svg.setAttribute('fill', this.backgroundColors[fill% this.backgroundColors.length]);
