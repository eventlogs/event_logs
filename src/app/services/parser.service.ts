...
export class ParserService {
    ...
    public static PARSING_ERROR = new Error("given .type log string can not be parsed")
    ...
    public parse(text: string): EventLog | undefined {
        ...
        const indexAttributes = ParserService.indexOfTokenIfExists(lines, this._attributesElement)
        const indexEvents = ParserService.indexOfTokenIfExists(lines, this._eventsElement)
        const max = lines.length
        const keywordIndices: number[] = [indexLog, indexAttributes, indexEvents, max]

        const attributesLines: string[] = lines.slice(indexAttributes + 1, ParserService.nextKeyword(keywordIndices, indexAttributes))
        const headers: string[] = attributesLines.map(attribute => attribute.trim())

        const eventLines: string[] = lines.slice(indexEvents + 1, ParserService.nextKeyword(keywordIndices, indexEvents))
    ...
    private parseTraces(headers: string[], eventLines: string[]): Trace[] {
        const asTable = eventLines
            .map(eventLine => eventLine.split(" ").map(attributeValue => attributeValue.trim()))

        const dictCaseIdentifierToTrace: Map<Number, Trace> = new Map()
        asTable.forEach(eventLine => {
            if (eventLine[headers.indexOf(this._caseIdElement)] === undefined ||
                eventLine[headers.indexOf(this._activityElement)] === undefined) {
                throw ParserService.PARSING_ERROR
            }

            const caseId: number = parseInt(eventLine[headers.indexOf(this._caseIdElement)])
            const activity: string = eventLine[headers.indexOf(this._activityElement)]
            ...
            const eventLogAttributes: EventLogAttribute[] = headers
                ...
                .map(header => ParserService.eventLogAttributeOf(header, eventLine[headers.indexOf(header)],))

            if (!dictCaseIdentifierToTrace.has(caseId)) {
                dictCaseIdentifierToTrace.set(caseId, new Trace([], [], caseId))
            }
            dictCaseIdentifierToTrace.get(caseId)?.events.push(new Event(eventLogAttributes, activity))
        })
    ...
    private static nextKeyword(keywordIndices: number[], actKeyWord: number): number {
    ...
    private static eventLogAttributeOf(key: string, value: string): EventLogAttribute {
        switch (value) {
            case "true":
