...
export class XesParserService {
    ...
    public static PARSING_ERROR = new Error("given xes string can not be parsed")
    ...
    private readonly _allXesElements = [this._stringAttributeToken, this._dateAttributeToken, this._intAttributeToken,
        this._floatAttributeToken, this._booleanAttributeToken]
    ...
    private convertToEventLog(result: any): EventLog {
        ...
        const extensions = this.convertToExtensions(logObj[this._extensionToken]) // TODO -> Nutzen ?? Aktuell Array, dass jeweils eine Extension mit Keys (NAME, PREFIX, URI) enthält
        ...
        return new EventLog(classifiers, globalEventAttributes, globalTraceAttributes, traces, logAttributes);
    }
    ...
    private convertToExtensions(extensionsObj: any): Map<string, string> {
        ...
        return extensionsObj
            .map((extensionsObj: any) => this.readElementsOfAttribute(extensionsObj))
    }
    ...
    private convertToClassifiers(classifiersObj: any): Classifier[] {
        ...
        return classifiersObj
            .map((classifierObj: any) => this.readElementsOfAttribute(classifierObj))
            .map((elementsMap: any) => new Classifier(
                elementsMap.get(this._nameToken),
                elementsMap.get(this._keysToken).split(" ")))
    }

    private convertToGlobalAttributes(scope: string, globalAttributes: any): EventLogAttribute[] {
        if (globalAttributes == null) {
            return []
        }
        const result = globalAttributes
            .filter((scopedAttributes: any) => this.readElementsOfAttribute(scopedAttributes).get(this._scopeToken) === scope)
            .map((scopedAttributes: any) => this.extractEventLogAttributes(scopedAttributes))[0]
    ...
    private convertToTraces(tracesObj: any): Trace[] {
        ...
        return tracesObj
            .map((traceObj: any, caseId: number) => this.convertToTrace(traceObj, caseId))
    ...
    convertToTrace(traceObj: any, caseId: number): Trace | undefined {
        ...
        const events: Event[] = this.convertToEvents(traceObj[this._eventToken])
    ...
    private convertToEvent(eventObj: any): Event | undefined {
        ...
        const activityArr = eventLogAttributes.filter(eventLogAttribute => eventLogAttribute.key.toLowerCase() == this._activityEventLogAttributeKey)
    ...
    private extractEventLogAttributesOfType(type: string, attributes: any): EventLogAttribute[] {
        if (attributes == null) {
            return []
        }
        const directAttributes = attributes
            .map((attribute: { [x: string]: any; }) => attribute[this._attributesToken])
            .map((attribute: { [x: string]: any; }) => this.buildAttribute(type, attribute[this._valueToken], attribute[this._keyToken]))
            .filter((attribute: EventLogAttribute | undefined) => attribute != null)
        const subAttributes = attributes
            .map((attribute: { [x: string]: any; }) => attribute[type])
            .flatMap((subAttribute: any) => this.extractEventLogAttributesOfType(type, subAttribute))
        return directAttributes.concat(subAttributes)
    }

    private buildAttribute(type: string, value: any, key: String): EventLogAttribute | undefined {
        switch (type) {
